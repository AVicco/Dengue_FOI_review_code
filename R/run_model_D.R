...
#Read your csv file 
df <-read.csv(paste0(wd0, "/data/dataset_modelD.csv"))
...
#BINOM MODEL
mod <- cmdstan_model('Seromodel_D.stan', pedantic=T)
...
traceplot(stanfit, c("lambda1", "lambda2", "lambda3", "lambda4", "lambda"))
fit$cmd_summary()

saveRDS(fit, paste0(wd0, "/output/fit_PRNT_", loc, ".rds"))
...
write.csv(stat, paste0(wd0, "/output/stat_PRNT_", loc, ".csv"))
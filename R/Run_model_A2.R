...
#########################################################
#
#             CODE TO RUN BETABINOMIAL LL MODEL A
#
#########################################################
...
#BINOM MODEL
mod <- cmdstan_model('Seromodel_A2.stan', pedantic=T)
...
traceplot(stanfit, c("lambda", "phi"))
fit$cmd_summary()

saveRDS(fit, paste0(wd0, "/output/fit_betabin_", loc, ".rds"))

#Calculate statistics from posterior distributions
val_lambda <- c(chains$lambda) #FOI
val_phi <- c(chains$phi) #overdispersion

x <- sample(seq(1,15000), 100)  #extract 100 random samples from lambda and phi
for (i in 1:length(x)){
...
stat <- rbind(lambda_stat, phi_stat, age_serop_50_stat, age_serop_70_stat,age_first_stat)
colnames(stat) <- c("median", "ciL", "ciU")
write.csv(stat, paste0(wd0, "/output/stat_betabin_", loc, ".csv"))
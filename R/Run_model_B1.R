...
#########################################################
#
#             CODE TO RUN BINOMIAL LL MODEL B
#
#########################################################
...
#BINOM MODEL
mod <- cmdstan_model('Seromodel_B1.stan', pedantic=T)
...
traceplot(stanfit, c("lambda", "alpha"))
fit$cmd_summary()

saveRDS(fit, paste0(wd0, "/output/fit_bin_age_", loc, ".rds"))
...
stat <- rbind(lambda_stat, alpha_stat, age_serop_50_stat, age_serop_70_stat,age_first_stat)
colnames(stat) <- c("median", "ciL", "ciU")
write.csv(stat, paste0(wd0, "/output/stat_bin_age_", loc, ".csv"))
